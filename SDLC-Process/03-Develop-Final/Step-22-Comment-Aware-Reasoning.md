...
- **Function and Method Behaviour:** Behaviour described in comments matches implementation.
- **Parameter and Return Expectations:** Parameters and return behaviour match what is documented.
- **Validation and Error Handling:** Validation and error-handling behaviour align with documented intent.
...
- **Undocumented Interfaces:** Public or shared internal interfaces without meaningful notes.
- **Undocumented Edge Cases:** Important edge-case handling with no context.
- **Missing Technical Constraints:** Non-obvious technical constraints not captured in code comments.
...
**DON'T:**
- **Compensate for Weak Code with Comments:** Do not use comments to compensate for poor naming, poor structure, or weak code clarity.


## 22.11 Summary and Key Outcomes
Step 22 checks that comments, inline documentation, and implementation intent remain aligned so that the change is easier to review, maintain, and support after delivery.

Key Outcomes:
- **Intent Alignment:** Comments and implementation behaviour remain consistent.
- **Improved Maintainability:** Low-value, stale, or missing documentation issues are reduced.
- **Clearer Review Context:** Reviewers receive a change set that is easier to understand without reconstructing intent.
- **Reduced Documentation Drift:** Outdated or misleading comments are corrected before broader review progresses.
- **Readability Readiness:** The change is better prepared for pull request formalisation and downstream review.
