...
| **Dead-letter stream (DLQ)** | Special log or queue where schema-violating, PII-violating, or otherwise invalid telemetry records are stored for diagnosis instead of silently dropped. | [Chapter 20. Observability Data Model Specification -> Section 20.9.6 Dead-Letter Discipline for Schema Violations](20-observability-data-model-specification.md#2096-dead-letter-discipline-for-schema-violations) |
...
| **Policy-as-code (for observability)** | Encoding observability invariants (e.g. correlation propagation, 5xx logging with context, cardinality budgets) as machine-evaluated policies in CI/CD, rather than relying on manual review. | [Chapter 8. IaC for Observability Standard -> Section 8.1.1 Policy-as-Code for Telemetry Invariants](08-iac-for-observability-standard.md#811-policy-as-code-for-telemetry-invariants) |
...
| **Service tier** | Criticality classification (T1–T4) that drives instrumentation depth, SLO strictness, retention, alerting policy, and on-call coverage. | [Chapter 2. Enterprise Observability Standards Catalogue -> Section 2.4.1 Service Tiering Model](02-enterprise-observability-standards-catalog.md#241-service-tiering-model), [Chapter 21. Business Capability and Value-Stream Mapping](21-business-capability-and-value-stream-mapping.md) |
...
| **Tail-based sampling** | Sampling decision made after the trace completes, allowing rules like "keep all traces with errors". More expensive but vastly more useful. | [Chapter 18. Application Telemetry Standard](18-application-telemetry-standard.md), [Chapter 3. Observability Reference Architecture](03-observability-reference-architecture.md) |
...
| **Telemetry health** | The degree to which telemetry itself is complete, correct, and timely (e.g. no stuck gauges, flat counters, missing traces, or skewed timestamps). | [Chapter 20. Observability Data Model Specification -> Section 20.8 Telemetry Health and Data-Quality Monitoring](20-observability-data-model-specification.md#208-telemetry-health-and-data-quality-monitoring), [Chapter 6. Grafana Platform Standard and Visualisation Playbook -> Section 6.7 Telemetry Health Dashboard Standard](06-grafana-platform-standard-and-visualisation-playbook.md#67-telemetry-health-dashboard-standard) |
