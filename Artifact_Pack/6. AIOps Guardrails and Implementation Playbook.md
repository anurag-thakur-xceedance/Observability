...
| **5. Model card** | Human-readable summary of intent, training data, evaluation, limits | Published model card (template §7.3) | AIOps Lead |
...
| **7. ARB approval** | Governance ratification | ADR entry in [Chapter 16](16.%20Observability%20ADR%20Decision%20Register.md) | ARB |
...
| **Cross-tenant leakage** | LLM context window contains tenant A's data when serving tenant B | Per-tenant isolation of retrieval index; tenant ID enforced in retrieval filter; never share embeddings across tenants ([Chapter 26](26.%20Multi-Tenant%20and%20Customer-Site%20Deployment%20Model.md)) | Platform Ops |
...
Telemetry sent to **any** LLM (hosted or self-hosted) must pass through the same redaction pipeline that governs telemetry storage ([Chapter 23. Observability Platform Security Architecture](23.%20Observability%20Platform%20Security%20Architecture.md)). Specifically:
...
5. **Audit log of all prompts.** Every prompt and response is logged with the operator's identity, retained per [Chapter 8](8.%20Observability%20Data%20Governance%20and%20Retention%20Policy.md), and subject to the same RBAC as other telemetry.

### 8.5 AI Safety KPIs

Reported on the scorecard in [Chapter 11](11.%20Observability%20KPI%20Scorecard.md) and reviewed monthly by the AIOps Lead with the governance body.
...
- **[Chapter 8. Observability Data Governance and Retention Policy](8.%20Observability%20Data%20Governance%20and%20Retention%20Policy.md)** — retention rules for prompt audit logs.
- **[Chapter 10. Compliance and Audit Control Matrix -> Section 5. Control Matrix (Initial)](10.%20Compliance%20and%20Audit%20Control%20Matrix.md#5-control-matrix-initial)** — OBS-C-14 audits the lifecycle in §7.
- **[Chapter 11. Observability KPI Scorecard](11.%20Observability%20KPI%20Scorecard.md)** — Phase 3 KPI targets (MTTD < 1 min, automated tickets > 90%); AI safety KPIs from §8.5.
...
- **[Chapter 16. Observability ADR Decision Register](16.%20Observability%20ADR%20Decision%20Register.md)** — ADR-016 governs LLM use; lifecycle gate Stage 7.
- **[Chapter 23. Observability Platform Security Architecture](23.%20Observability%20Platform%20Security%20Architecture.md)** — redaction pipeline applied to LLM prompts.
- **[Chapter 26. Multi-Tenant and Customer-Site Deployment Model](26.%20Multi-Tenant%20and%20Customer-Site%20Deployment%20Model.md)** — tenant isolation for LLM retrieval.
