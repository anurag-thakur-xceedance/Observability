...
| **R**epudiation | Configuration changes | No actor attribution | Audit-event schema (§5); SIEM forwarding |
| **I**nformation Disclosure | Logs containing PII | Unredacted log lines stored | OTel Collector `attributes/redact` + `transform` processors; pattern catalog (§4); Loki line filters |
