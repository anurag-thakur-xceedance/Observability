...
## 8. Canonical JSON Schemas, ERD, and OTel Crosswalk
...
- Schema changes follow the change-control process in [Chapter 15. Observability Governance Charter and ARB Pack -> Section 3. Decision Rights](15-observability-governance-charter-and-arb-pack.md#3-decision-rights).
...
- A **Service** depends on zero-or-many **Dependencies** and is depended upon by zero-or-many other Services (the dependency graph derived from span `parent`/`child` relationships per [Chapter 17. Application Telemetry Standard -> Section 4. Post-Login Telemetry](17-application-telemetry-standard.md#4-post-login-telemetry-required-fields-standards)).
- A **Transaction** spans one-or-many **Services** (root + child spans); a Service participates in zero-or-many concurrent Transactions.
- An **Incident** correlates with zero-or-many **Deployments** and **Scaling Events** within its detection window (default ±30 min, governed by [Chapter 6. AIOps Guardrails and Implementation Playbook -> Section 4. Severity Policy for AI-Detected Events](06-aiops-guardrails-and-implementation-playbook.md#4-severity-policy-for-ai-detected-events)).

### 8.3 OpenTelemetry Semantic-Convention Crosswalk
...
| Named user journey        | `attributes.transaction.name`, `attributes.journey.name` | *Xceedance extension* | See [Chapter 17. Application Telemetry Standard -> Section 4. Post-Login Telemetry](17-application-telemetry-standard.md#4-post-login-telemetry-required-fields-standards). |
...
### 8.4 Dead-Letter Discipline for Schema Violations
...
|---|---|---|---|
| **Required-field-missing** | Resource block missing `service.name`, `service.version`, or `deployment.environment`. | Loki tenant `dlq-required` (7-day retention). | Counted against the producing service's *data-completeness* SLO ([Chapter 8. Observability Data Governance and Retention Policy -> Section 6. Data Quality and Standards](08-observability-data-governance-and-retention-policy.md#6-data-quality-and-standards)). |
...
### 8.5 Mapping to Native Backend Formats
...
### 8.6 Conformance and Reference Implementations
...
- [Chapter 6. AIOps Guardrails and Implementation Playbook -> Section 3. Interpreting the AI-Driven Metrics](06-aiops-guardrails-and-implementation-playbook.md#3-interpreting-the-ai-driven-metrics) — consumer of the ERD in Section 8.2.
...
- [Chapter 23. Observability Platform Security Architecture -> Section 4. PII Redaction (Concrete Mechanisms)](23-observability-platform-security-architecture.md#4-pii-redaction-concrete-mechanisms) — PII patterns feeding the `dlq-pii` stream.
- [Chapter 15. Observability Governance Charter and ARB Pack -> Section 3. Decision Rights](15-observability-governance-charter-and-arb-pack.md#3-decision-rights) — schema-version change-control authority.
