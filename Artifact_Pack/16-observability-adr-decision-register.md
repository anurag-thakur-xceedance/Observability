...
## 4. ADR Bodies (Full Records)

### ADR-000: Observability Strategy Principles (10 principles)
...
- **Reviewers**: ARB
- **Supersedes**: —
- **Related**: [Chapter 0 -> Section 1.1](0-observability-strategy-revised.md#11-vision-mission-and-guiding-principles)
...
### ADR-001: Adopt OpenTelemetry as the universal telemetry standard
...
- **Supersedes**: —
- **Related**: [Chapter 2](2-observability-reference-architecture.md), [Chapter 17](17-application-telemetry-standard.md), [Chapter 19](19-observability-data-model-specification.md)
...
### ADR-002: Select Grafana as primary visualization & alerting tool
...
- **Reviewers**: ARB
- **Supersedes**: —
- **Related**: [Chapter 5](5-grafana-platform-standard-and-visualization-playbook.md)
...
### ADR-003: Use Prometheus / Loki / Tempo as backends
...
- **Reviewers**: ARB
- **Supersedes**: —
- **Related**: [Chapter 2](2-observability-reference-architecture.md), [Chapter 8](8-observability-data-governance-and-retention-policy.md)
...
### ADR-004: Adopt PowerShell + Docker Compose for IaC and orchestration (supersedes Pulumi/Kubernetes)
...
- **Related**: [Chapter 7](7-iac-for-observability-standard.md), ADR-005
...
### ADR-005: Host-portable deployment (supersedes multi-cloud K8s posture)
...
- **Related**: [Chapter 2 -> Section 6](2-observability-reference-architecture.md#6-host-portable-deployment-design), [Chapter 26](26-multi-tenant-and-customer-site-deployment-model.md), ADR-004
...
### ADR-006: Tiered retention (hot / warm / cold) with policy schedule
...
- **Supersedes**: —
- **Related**: [Chapter 8](8-observability-data-governance-and-retention-policy.md), [Chapter 28](28-observability-long-term-archival-policy.md)
...
### ADR-007: AIOps guardrails (FP < 5%, detection latency < 2 min)
...
- **Supersedes**: —
- **Related**: [Chapter 6](6-aiops-guardrails-and-implementation-playbook.md)
...
### ADR-008: Adopt Sloth as SLO-rule generator
...
- **Supersedes**: —
- **Related**: [Chapter 24 -> Section 8](24-slo-and-error-budget-framework.md#8-tooling-decision)
...
### ADR-009: Service Tiering Model (T1–T4)
...
- **Supersedes**: —
- **Related**: [Chapter 1 -> Section 4.1](1-enterprise-observability-standards-catalog.md#41-service-tiering-model)
...
### ADR-010: Cardinality Budget (per-service, enforced at SDK + collector + backend)
...
- **Supersedes**: —
- **Related**: [Chapter 1 -> Section 3.1](1-enterprise-observability-standards-catalog.md#31-cardinality-governance), [Chapter 22 -> Section 8](22-capacity-and-scale-model.md#8-cardinality-budget)
...
### ADR-011: Multi-tenant data isolation with gateway-injected `tenant_id`
...
- **Supersedes**: —
- **Related**: [Chapter 26 -> Section 3](26-multi-tenant-and-customer-site-deployment-model.md#3-tenant-labelling-enforcement)
...
### ADR-012: Auto-instrumentation via eBPF (Beyla) for legacy services
...
- **Supersedes**: —
- **Related**: [Chapter 2 -> Section 4.1](2-observability-reference-architecture.md#41-auto-instrumentation-via-ebpf)
...
### ADR-013: Tail-based sampling at gateway with 100%-error retention
...
- **Supersedes**: —
- **Related**: [Chapter 2 -> Section 5.1](2-observability-reference-architecture.md#51-sampling-strategy)
...
### ADR-014: HA topology (Prometheus pair, AM 3-cluster, Loki/Tempo + object store, Grafana × 2 + external Postgres)
...
- **Supersedes**: —
- **Related**: [Chapter 21 -> Section 3](21-observability-platform-ha-and-dr-design.md#3-reference-ha-topology-compose-single-region)
...
### ADR-015: Compliance framework adoption (SOC 2 Type II + ISO 27001 + GDPR + DORA-where-applicable)
...
### ADR-016: LLM telemetry-data handling (redact-before-prompt, tenanted LLM only, no third-party access to raw telemetry)
...
- **Supersedes**: —
- **Related**: [Chapter 6 -> Section 8](6-aiops-guardrails-and-implementation-playbook.md#8-ai-safety-explainability-and-llm-data-leakage), Chapter 23 Appendix A residual risk **C7-I**
