# D4 — Alerting & Incident Severity Policy
...
- Alert rules and dashboards are version-controlled (GitOps). See **D8 §11.5** equivalent and **D10**.
- Severity-based routing is configured in Grafana Alerting / Alertmanager.
- Each alert is associated with a runbook reference (see **D3** and **D12**).
- Quiet hours, deduplication, and grouping are tuned to maintain signal-to-noise ratio ≥ 80% (see **D11**).
...
## 7. Cross-References
- **D1** — threshold definitions feeding alert rules.
- **D3** — runbooks invoked by alerts.
- **D5** — Grafana implementation of alerting.
- **D6** — AI-specific alerting tuning.
- **D11** — alert quality KPIs (signal-to-noise ratio).
- **D12** — incident response workflow once alert fires.
