...
| Cardinality drift review ([Chapter 1 §3.1](1.%20Enterprise%20Observability%20Standards%20Catalog.md#31-cardinality-governance)) | Weekly | Platform Ops | Active-series report |
...
### 9a.2 Routine Procedures

**Adding a new metric or label.** Service team submits PR to schema repo → Data Governance reviews against `1. §3.1` budget → cardinality estimate computed → if within budget, merge; if not, requires governance body waiver and budget reallocation.
...
**Retention change.** Data Governance proposes change → Platform Ops models cost and capacity impact ([Chapter 22](22.%20Capacity%20and%20Scale%20Model.md)) → governance body approves → Platform Ops applies via IaC ([Chapter 7](7.%20IaC%20for%20Observability%20Standard.md)) → post-change validation per [Chapter 7 §7.1.5](7.%20IaC%20for%20Observability%20Standard.md#71-change-management).

**Tenant offboarding ([Chapter 26](26.%20Multi-Tenant%20and%20Customer-Site%20Deployment%20Model.md)).** Data Governance issues deletion ticket → Platform Ops runs scoped deletion against each backend (Prometheus tenant labels, Loki tenant streams, Tempo tenant blocks) → object-storage tenant prefix marked for deletion (lifecycle rule honours legal hold) → certificate of deletion produced for the tenant.

### 9a.3 Operational Toil Targets
...
### 9a.4 Knowledge Continuity

To mitigate **GOV-R-10** (single-point-of-knowledge), every Day-2 procedure has:
- A runbook in [Chapter 3](3.%20Domain%20Observability%20Runbooks%20Pack.md) or in the platform's internal runbook library.
...
- **[Chapter 3. Domain Observability Runbooks Pack](3.%20Domain%20Observability%20Runbooks%20Pack.md)** — runbooks for §9.1 Day-2 procedures.
- **[Chapter 7. IaC for Observability Standard -> Section 7.1. Change Management](7.%20IaC%20for%20Observability%20Standard.md#71-change-management)** — change workflow used for §9.1 retention changes.
- **[Chapter 9. Observability FinOps Standard](9.%20Observability%20FinOps%20Standard.md)** — cost-driven lifecycle mechanics.
- **[Chapter 10. Compliance and Audit Control Matrix](10.%20Compliance%20and%20Audit%20Control%20Matrix.md)** — compliance and audit control matrix; OBS-C-04, OBS-C-08 sourced from §9.1.
...
- **[Chapter 22. Capacity and Scale Model](22.%20Capacity%20and%20Scale%20Model.md)** — capacity inputs to §9.1.1 weekly review.
- **[Chapter 26. Multi-Tenant and Customer-Site Deployment Model](26.%20Multi-Tenant%20and%20Customer-Site%20Deployment%20Model.md)** — tenant offboarding procedure cross-ref.
