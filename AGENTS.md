...
### 1.1 Cross-references — never use `§`
Do **not** use the section sign `§` anywhere in the pack — neither in prose, nor in tables, nor in Mermaid node labels, nor (especially) inside link labels.
...
- In prose, write `Section 3.2`, not `§3.2` or `Sec 3.2`.
- In Mermaid node labels where space is tight, use `Sec 7.2` (3 chars) — still no `§`.
...
1. `Get-ChildItem Artifact_Pack -Filter *.md -Recurse | Select-String -Pattern 'TEMP|TODO|FIXME|XXX|PLACEHOLDER|§'` → must return zero hits except `TBD` in `owner:` fields.
...
- The shorthand `§N` crept into cross-reference link labels (e.g. `[Chapter 27 §3. NFR Register](...)`) and tables, inconsistent with the pack's own `Section N` style.
